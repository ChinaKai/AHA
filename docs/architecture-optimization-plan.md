...
| Stop/cleanup subprocesses | P3.2 ✅ done | backend/browser/observe/headroom/serial/knowledge/local-terminal stop now cross-platform |
| Temp dirs / cleanup | P3.3 ✅ done | ready |
| Local terminal shell | P3.3 + P3.9 ✅ done | POSIX PTY retained; Windows uses native ConPTY for UTF-8 VT I/O, incremental input, line editing, control keys, and resize. A host-codepage pipe session remains as the compatibility fallback when ConPTY is unavailable. |
...
| 2026-08-01 | Done | **Windows Integrations local terminal + Daily Usage landed.** `LocalTerminalSession` uses native Windows ConPTY with UTF-8 VT I/O, incremental input, line editing/control keys, resize, clean process termination, and Job Object binding; a redirected host-codepage pipe session remains for Windows versions without ConPTY. Real Windows testing entered a command one character at a time, corrected it with Backspace, resized the console, and executed only the corrected command. Daily Usage accepts `UTC` and browser IANA timezone names when Windows has no Python `tzdata`, uses host-local date only for validation, and normalizes npm shim spawning; real ccusage test returned one day and 1,704,362 tokens for `Asia/Shanghai`. |
| 2026-08-01 | Done | **Local Terminal shell picker landed.** Added host-level detection and a persisted picker for Auto / PowerShell 7 / Windows PowerShell / Command Prompt / WSL. Only fixed shell IDs cross the WebSocket boundary; WSL is offered only with a registered distribution, and Auto prefers `pwsh` → Windows PowerShell → CMD. Real 8789 testing launched Auto, PowerShell, CMD, and WSL through ConPTY and verified a command marker from each. |
