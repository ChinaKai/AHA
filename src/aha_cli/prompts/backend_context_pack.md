...
- Related knowledge projects are read-only task snapshots created from Project Identity relations. Use only their direct entrypoints when relevant, do not recursively expand to other projects, and prefer the current source and current-project KB when knowledge conflicts.
...
- Non-index navigation documents must keep frontmatter and physical paths aligned: `navigation/modules/<name>.md` uses `"slug":"modules/<name>"` and `"navigation_role":"module"`; `navigation/flows/<name>.md` uses `"slug":"flows/<name>"` and `"navigation_role":"flow"`. Never use a bare child slug such as `"gateway"`.
- Navigation `navigation_role` values are `index`, `module`, or `flow`, matching the `slug` form above.
- Navigation-internal Markdown links must use full navigation slugs: `](modules/<name>.md)`, `](flows/<name>.md)`, or `](index.md)`. Do not use `../index.md` or a bare `<name>.md`; the Knowledge Web UI resolves these links by frontmatter `slug`, not filesystem-relative paths.
- A custom navigation `id` is optional. The normalized `slug` is the Web/API lookup and internal-link key, so it must be present and correct even when `id` is omitted.
