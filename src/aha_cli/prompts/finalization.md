...
- Each candidate must be reusable knowledge, not a copy of the Final. Ordinary one-off bug fixes should usually produce no candidate; project map discoveries should update navigation/module docs instead of becoming solution entries. Use this JSON shape:
...
- For `kind="navigation"`, emit when this task changed OR usefully discovered/validated project-map facts: a module's responsibility, entry point, architecture, key source locations, reusable diagnostic path, old navigation blind spot, missing module/flow entry, or stale/incorrect nav. Read-only diagnostics count when they reveal where future agents should start. Navigation updates are incremental: update only the affected `"index"`, `"modules/<module-slug>"`, `"modules/<module>/<child-slug>"`, `"flows/<flow-slug>"`, or `"flows/<flow>/<child-slug>"` document, never regenerate the whole project navigation from an ordinary task. Each navigation document only owns one layer of links: index links first-level modules/flows, module/flow docs link only their direct children. If root index is missing, AHA will bootstrap it from the workspace; if a non-root child doc has no direct parent entry, AHA will add a minimal parent-link candidate. Do not expand grandchildren into index. Carry forward still-correct content and only adjust what changed. Prefer fields such as `"slug"`, `"module"`, `"responsibility"`, `"related_files"`, `"entry_points"`, `"diagnostic_paths"`, and `"navigation_reason"` so the delta is reviewable.
