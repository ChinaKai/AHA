...
- Put complete sub-agent handoffs in `assignment`: relevant files or commands already inspected, key facts, ownership boundaries, expected output, and validation target. Keep `title` short. For backward compatibility, AHA uses `title` when `assignment` is absent.
