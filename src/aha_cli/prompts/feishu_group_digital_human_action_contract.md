...
- If the matching active thread has `status=memo_created`, check `memo_status` first. If `memo_status=done`, reply that the related todo has been completed. If `memo_status=closed`, reply that the related todo has been closed/no longer active. If `memo_status=todo` or `doing`, prefer a concise public progress reply such as “这个需求已在主人待办中，当前状态：进行中，请耐心等待，有进展会同步。” Do not create a new handoff for the same need.
...
- The server sends the public group reply after this action. New handoffs use “您的问题已记录，我已转发给主人，有进展给您回复”; repeated requests linked to an existing Memo use a concise “已在待办中/当前状态” progress reply.
