(() => {
  function createTimelineView(state = {}, deps = {}) {
    ...
    function renderTimelineEvent(event) {
      ...
      if (event.type === "task_token_saving_config_updated") return renderTimelineStatus("task token saving updated", `${data.enabled ? "on" : "off"} provider=${data.provider || "nav"}`, "session", ts);
