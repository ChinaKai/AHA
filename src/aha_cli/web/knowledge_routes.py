...
from aha_cli.services.project_context_index_jobs import (
...
from aha_cli.web.http_utils import http_response, json_response, parse_json_body, parse_query_bool
...
def _public_project_context_status(status: dict) -> dict:
    item = dict(status)
    item.pop("index", None)
    return item


def _public_project_context_job(job: dict) -> dict:
    item = dict(job)
    item.pop("_path", None)
    return item
...
_PROJECT_CONTEXT_FILE_READ_LIMIT = 2 * 1024 * 1024
...
def _project_context_status_for_request(
    root: Path,
    cfg: dict,
    payload: dict,
    ...
    *,
    ...
    status = project_context_index_status(
        root,
    ...
    return context, project_key_value, status


def _project_context_map_dir(status: dict) -> Path:
    paths = status.get("paths") if isinstance(status.get("paths"), dict) else {}
    raw = str(paths.get("dir") or "").strip()
    if not raw:
        raise FileNotFoundError("project context map is missing")
    base = Path(raw).expanduser().resolve()
    if not status.get("exists") or not base.exists() or not base.is_dir():
        raise FileNotFoundError("project context map is missing")
    return base


def _project_context_runtime_root(root: Path) -> Path:
    return aha_home_path(root) / "runtime" / "project_context"


def _project_context_map_dir_from_ref(root: Path, project_key_value: str, workspace_id: str) -> Path:
    project_key_value = str(project_key_value or "").strip()
    workspace_id = str(workspace_id or "").strip()
    if not project_key_value or not workspace_id:
        raise FileNotFoundError("project context map reference required")
    runtime_root = _project_context_runtime_root(root).resolve()
    base = project_context_index_dir(root, project_key_value, workspace_id).resolve()
    try:
        base.relative_to(runtime_root)
    except ValueError as exc:
        raise PermissionError("project context map reference escapes runtime directory") from exc
    if not base.exists() or not base.is_dir() or not (base / "index.json").exists():
        raise FileNotFoundError("project context map is missing")
    return base


def _project_context_map_dir_for_query(
    root: Path,
    cfg: dict,
    query: dict[str, list[str]],
) -> tuple[Path, dict, str, str]:
    project_key_value = _payload_value({}, query, "project_key", "project")
    workspace_id = _payload_value({}, query, "workspace_id")
    if project_key_value and workspace_id:
        base = _project_context_map_dir_from_ref(root, project_key_value, workspace_id)
        try:
            manifest = read_json(base / "index.json")
        except (OSError, ValueError):
            manifest = {}
        return base, manifest if isinstance(manifest, dict) else {}, project_key_value, workspace_id
    context, project_key_value, status = _project_context_status_for_request(
        root,
        cfg,
        {},
        query,
    ...
    base = _project_context_map_dir(status)
    manifest = status.get("index") if isinstance(status.get("index"), dict) else {}
    return base, manifest, project_key_value, str(status.get("workspace_id") or context.get("workspace_id") or "")


def _project_context_map_summary(index_path: Path) -> dict | None:
    try:
    ...
    return {
        "project_key": manifest.get("project_key") or index_path.parent.parent.name,
        "workspace_id": manifest.get("workspace_id") or index_path.parent.name,
        "workspace": manifest.get("workspace") or "",
        "generated_at": manifest.get("generated_at") or "",
        "git_head": manifest.get("git_head") or "",
        "flavors": manifest.get("flavors") if isinstance(manifest.get("flavors"), list) else [],
...
def _list_project_context_maps(root: Path) -> list[dict]:
    runtime_root = _project_context_runtime_root(root)
    if not runtime_root.exists():
        return []
    maps: list[dict] = []
    for index_path in runtime_root.glob("*/*/index.json"):
        item = _project_context_map_summary(index_path)
        if item is not None:
            maps.append(item)
    return sorted(
        maps,
        key=lambda item: (str(item.get("generated_at") or ""), str(item.get("project_key") or "")),
        reverse=True,
    )


def _project_context_file_tree(base: Path) -> list[dict]:
    entries: list[dict] = []
...
def _resolve_project_context_file(base: Path, relpath: str) -> Path:
    clean = str(relpath or "").strip().lstrip("/")
    if not clean:
        raise FileNotFoundError("project context map file required")
    target = (base / clean).resolve()
    try:
        target.relative_to(base)
    except ValueError as exc:
        raise PermissionError("project context map file path escapes map directory") from exc
    if not target.exists() or not target.is_file():
        raise FileNotFoundError(f"project context map file not found: {clean}")
    return target
...
def knowledge_route_response(
    ...
    if method in {"GET", "HEAD"} and path == "/api/kb/project-context-index":
        try:
        ...
        try:
            verify_worktree = parse_query_bool(query, "deep", False)
        except ValueError as exc:
            return json_response({"error": str(exc)}, "400 Bad Request")
        status = project_context_index_status(
            root,
        ...
        return _ok(method, {
            "ok": True,
            "status": _public_project_context_status(status),
    ...
    if method in {"GET", "HEAD"} and path == "/api/kb/project-context-index/maps":
        maps = _list_project_context_maps(root)
        return _ok(method, {"ok": True, "maps": maps, "count": len(maps)})

    if method in {"GET", "HEAD"} and path == "/api/kb/project-context-index/tree":
        try:
            base, manifest, project_key_value, workspace_id = _project_context_map_dir_for_query(root, cfg, query)
        except FileNotFoundError as exc:
            return json_response({"error": str(exc)}, "404 Not Found")
        ...
        return _ok(method, {
            "ok": True,
            "root": str(base),
            "entries": _project_context_file_tree(base),
            "map": _project_context_map_summary(base / "index.json") or {},
            "workspace_path": manifest.get("workspace") or "",
            "project_key": project_key_value,
            "workspace_id": workspace_id,
        })

    if method in {"GET", "HEAD"} and path == "/api/kb/project-context-index/file":
        try:
            base, manifest, project_key_value, workspace_id = _project_context_map_dir_for_query(root, cfg, query)
            target = _resolve_project_context_file(base, _payload_value({}, query, "path", "file"))
        except PermissionError as exc:
            return json_response({"error": str(exc)}, "400 Bad Request")
        except FileNotFoundError as exc:
            return json_response({"error": str(exc)}, "404 Not Found")
        ...
        try:
            ...
            with target.open("rb") as handle:
                raw = handle.read(_PROJECT_CONTEXT_FILE_READ_LIMIT + 1)
        except OSError as exc:
            return json_response({"error": str(exc)}, "500 Internal Server Error")
        truncated = len(raw) > _PROJECT_CONTEXT_FILE_READ_LIMIT or size > _PROJECT_CONTEXT_FILE_READ_LIMIT
        raw = raw[:_PROJECT_CONTEXT_FILE_READ_LIMIT]
        return _ok(method, {
            "ok": True,
    ...
    if method == "DELETE" and path == "/api/kb/project-context-index/map":
        payload = parse_json_body(body) if body.strip() else {}
        project_key_value = _payload_value(payload, query, "project_key", "project")
        workspace_id = _payload_value(payload, query, "workspace_id")
        try:
            base = _project_context_map_dir_from_ref(root, project_key_value, workspace_id)
        except FileNotFoundError as exc:
            return json_response({"error": str(exc)}, "404 Not Found")
    ...
    if method == "POST" and path == "/api/kb/project-context-index/refresh":
        payload = parse_json_body(body) if body.strip() else {}
        try:
            context = _project_nav_bootstrap_context(root, payload, query)
        except FileNotFoundError as exc:
        ...
        job = start_project_context_refresh_job(
            root,
            workspace_path=context["workspace_path"],
            project_key_value=project_key_value,
            config=cfg,
        ...
        return json_response({
            ...
            "job_id": job["id"],
            "job": _public_project_context_job(job),
            "workspace_path": context["workspace_path"],
            "project_key": project_key_value,
            "run_id": context.get("run_id"),
            "task_id": context.get("task_id"),
    ...
    if method in {"GET", "HEAD"} and path == "/api/kb/project-context-index/job":
        job_id = str(query.get("id", [""])[0] or query.get("job_id", [""])[0] or "").strip()
        if not job_id:
            return json_response({"error": "job id required"}, "400 Bad Request")
        job = read_project_context_job(root, job_id)
        if job is None:
            return json_response({"error": f"project context index job not found: {job_id}"}, "404 Not Found")
        return _ok(method, {"ok": True, "job": _public_project_context_job(job)})

    if method == "POST" and path == "/api/kb/project-context-index/job/stop":
        payload = parse_json_body(body) if body.strip() else {}
        job_id = str(payload.get("id") or payload.get("job_id") or "").strip()
        if not job_id:
            return json_response({"error": "job id required"}, "400 Bad Request")
        try:
            job = stop_project_context_refresh_job(root, job_id)
        except FileNotFoundError:
            return json_response({"error": f"project context index job not found: {job_id}"}, "404 Not Found")
        return json_response({"ok": True, "job": _public_project_context_job(job), "stopped": bool(job.get("stopped"))})

    if method == "POST" and path == "/api/kb/project-context-index/query":
        payload = parse_json_body(body) if body.strip() else {}
        query_text = _payload_value(payload, query, "query", "q", "terms")
        if not query_text:
            return json_response({"error": "query required"}, "400 Bad Request")
        ref_project_key = _payload_value(payload, query, "project_key", "project")
        ref_workspace_id = _payload_value(payload, query, "workspace_id")
        if ref_project_key and ref_workspace_id:
            try:
                base = _project_context_map_dir_from_ref(root, ref_project_key, ref_workspace_id)
            except FileNotFoundError as exc:
                return json_response({"error": str(exc)}, "404 Not Found")
            except PermissionError as exc:
                return json_response({"error": str(exc)}, "400 Bad Request")
            ...
            if query_result is None:
                return json_response({
                    "ok": False,
                    "error": "project context index missing; refresh it first",
                    "status": {"status": "missing", "project_key": ref_project_key, "workspace_id": ref_workspace_id},
                    "workspace_path": manifest.get("workspace") or "",
                    "project_key": ref_project_key,
            ...
            status = {
                ...
                "workspace_id": ref_workspace_id,
                "workspace": manifest.get("workspace") or "",
                "paths": {"dir": str(base), "index": str(base / "index.json")},
                "counts": manifest.get("counts") if isinstance(manifest.get("counts"), dict) else {},
                "flavors": manifest.get("flavors") if isinstance(manifest.get("flavors"), list) else [],
                "profiles": manifest.get("profiles") if isinstance(manifest.get("profiles"), list) else [],
