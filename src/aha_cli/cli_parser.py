...
def build_parser(handlers: Mapping[str, Callable[[argparse.Namespace], int]]) -> argparse.ArgumentParser:
    ...
    kb_map = kb_sub.add_parser("map", help="Project navigation map (memory palace)")
    kb_map_sub = kb_map.add_subparsers(dest="kb_map_cmd", required=True)
    kb_map_build = kb_map_sub.add_parser("build", help="Scan a workspace into a project-map candidate")
