...
class CliCoreTests(unittest.TestCase):
    ...
    def test_prompts_include_commit_ownership_policy(self) -> None:
        ...
        self.assertIn("Execution constraints:", assignment_prompt)
        self.assertIn("- preferred_sub_model: env:work", assignment_prompt)
        self.assertIn("env:work", assignment_prompt)
        self.assertNotIn("Commit ownership policy:", assignment_prompt)
        self.assertNotIn("route it to that sub-agent with `route_to_agent`", assignment_prompt)
        self.assertNotIn("Never ask a sub-agent to commit files outside its assignment", assignment_prompt)
        self.assertNotIn("Commit message policy:", assignment_prompt)
        self.assertNotIn("Generated-by: AHA Codex GPT-5.5", assignment_prompt)
        self.assertNotIn("Include a concise commit body", assignment_prompt)
        self.assertNotIn("last non-empty line", assignment_prompt)
        self.assertNotIn("Keep task, agent, and scope tracking in the AHA journal", assignment_prompt)
        self.assertNotIn("AHA-Task: task-001", assignment_prompt)
        self.assertIn("Return one JSON object", assignment_prompt)
        self.assertIn("`actions`", assignment_prompt)
        self.assertNotIn("Completed, stopped, failed, interrupted, or blocked", assignment_prompt)
        self.assertNotIn("Include a stable `scope_id`", assignment_prompt)
        self.assertIn("For a brand-new sub-agent", assignment_prompt)
        self.assertNotIn("Include `agent_id` in `spawn_sub` only when intentionally reusing", assignment_prompt)
        self.assertNotIn("Collaboration mode:", assignment_prompt)
        self.assertNotIn("Workflow template:", assignment_prompt)
        self.assertIn("- workflow_template: fault-debug", assignment_prompt)
        self.assertNotIn("Fault debug:", assignment_prompt)
        self.assertNotIn("agent owns the efficiency decision", assignment_prompt)
        self.assertNotIn("never split work just to use more agents", assignment_prompt)
        self.assertNotIn("Spend the first 60 seconds decomposing", assignment_prompt)
        self.assertNotIn("optimize for end-to-end efficiency", assignment_prompt)
        self.assertNotIn("reduce the critical path", assignment_prompt)
        self.assertNotIn("simple or tightly coupled work", assignment_prompt)
        self.assertNotIn("raise your parallelism sensitivity", assignment_prompt)
        self.assertNotIn("state the practical reason briefly", assignment_prompt)
        self.assertIn("clear file ownership", assignment_prompt)
        self.assertIn("task-main responsible for integration, final review, and verification", assignment_prompt)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch("pathlib.Path.cwd", return_value=root):
                ...
                self.assertIn("AHA coordination policy:", main_prompt)
                self.assertIn("Action formats:", main_prompt)
                ...
                self.assertNotIn("Completed, stopped, failed, interrupted, or blocked", main_prompt)
                self.assertNotIn("Include a stable `scope_id`", main_prompt)
                self.assertNotIn("Spawn/reassign format:", main_prompt)
                self.assertNotIn("- preferred_sub_model:", main_prompt)
                self.assertIn('"model":null', main_prompt)
                self.assertNotIn("spend the first 60 seconds decomposing", main_prompt)
                self.assertNotIn("optimize for end-to-end efficiency", main_prompt)
                self.assertNotIn("reduce the critical path", main_prompt)
                self.assertNotIn("simple or tightly coupled work", main_prompt)
                self.assertNotIn("Do not split work just to use more agents", main_prompt)
                self.assertNotIn("raise your parallelism sensitivity", main_prompt)
                self.assertNotIn("state the practical reason briefly", main_prompt)
                self.assertIn("disjoint ownership", main_prompt)
                self.assertIn("task-main remains responsible for integration and final verification", main_prompt)
    ...
    def test_task_dashboard_and_metadata_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch("pathlib.Path.cwd", return_value=root):
                ...
                self.assertIn('data-bootstrap-config-field="proxy.http_proxy"', bootstrap_script)
                self.assertIn('data-bootstrap-config-field="proxy.https_proxy"', bootstrap_script)
                self.assertIn('data-bootstrap-config-field="proxy.no_proxy"', bootstrap_script)
