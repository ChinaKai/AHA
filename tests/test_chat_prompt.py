...
class ChatPromptTests(unittest.TestCase):
    ...
    def test_chat_prompt_includes_enabled_hardware_debug_context(self) -> None:
        ...
        self.assertIn("mode: groups", prompt)
        self.assertIn("hardware group: id=default, mode=both", prompt)
        self.assertIn("access permission: read_write", prompt)
        self.assertIn("serial: device=/dev/ttyUSB0", prompt)
        self.assertIn("network: device_ip=192.168.1.20", prompt)
        ...
        self.assertIn("hardware group: id=power, mode=serial", prompt)
        self.assertIn("description: Board power", prompt)
        self.assertIn("device=COM7", prompt)
        self.assertIn("--hardware <id>", prompt)
        ...
        self.assertIn("The first path is opened by the backend process", prompt)
        self.assertIn("In a WSL backend, use a WSL-visible path", prompt)
        self.assertIn("do not rewrite a", prompt)
        self.assertIn("raw `/home/...` path is not a Windows filesystem path", prompt)
    ...
    def test_chat_prompt_uses_context_pack_when_token_saving_enabled(self) -> None:
        ...
        self.assertIn("Do not use YAML frontmatter", enabled_prompt)
        self.assertIn("Every approved project navigation Markdown frontmatter must explicitly include", enabled_prompt)
        self.assertIn('`navigation/index.md` must contain `"slug":"index"`', enabled_prompt)
        self.assertIn('`"navigation_role":"index"`', enabled_prompt)
        self.assertIn('`navigation/modules/<name>.md` uses `"slug":"modules/<name>"`', enabled_prompt)
        self.assertIn('`navigation/flows/<name>.md` uses `"slug":"flows/<name>"`', enabled_prompt)
        self.assertIn("Never use a bare child slug", enabled_prompt)
        self.assertIn("`index`, `module`, or `flow`", enabled_prompt)
        self.assertIn("Do not use `../index.md` or a bare `<name>.md`", enabled_prompt)
        self.assertIn("The normalized `slug` is the Web/API lookup and internal-link key", enabled_prompt)
        self.assertIn("reachable parent-child hierarchy", enabled_prompt)
    ...
    def test_context_pack_does_not_inject_keyword_matched_knowledge_entries(self) -> None:
        ...
        self.assertIn("must use one JSON object frontmatter", prompt)
        self.assertIn("Navigation hierarchy: keep index as the top-level router", prompt)
        self.assertIn("reachable through direct parent links", prompt)
        self.assertIn("Write project-scoped KB Markdown", prompt)
        self.assertIn("in Chinese by default", prompt)
        self.assertIn("directly edit the approved KB Markdown files", prompt)
        self.assertIn("then update or create the project navigation entry with the verified files", prompt)
    ...
    def test_context_pack_does_not_reinject_prior_task_evidence(self) -> None:
        ...
        self.assertIn("task_worklog_frontmatter_json:", prompt)
        self.assertIn('"type":"task_worklog"', prompt)
        self.assertIn("reachable parent-child hierarchy", prompt)
        self.assertIn("in Chinese by default", prompt)
        self.assertIn("directly edit the approved KB Markdown files", prompt)
    ...
    def test_sticky_delta_injects_runtime_context_delta_once_for_task_capability_changes(self) -> None:
        ...
        self.assertIn("task_memo_assets/example.png", prompt)
        self.assertIn("serial: device=/dev/ttyUSB0", prompt)
        self.assertIn("login username: u", prompt)
